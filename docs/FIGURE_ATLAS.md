...
| Failure-aware VPPV evidence | `reports/figures/failure_aware_vppv/` | VPPV step evidence, severity holdout, mixed-priority audit, behavior-derived route assignment, and true mixed-fault rollout figures. |
...
| Can routes be derived from rollout behavior regions? | [Behavior-derived PCA](../reports/figures/failure_aware_vppv/failure_aware_vppv_model_derived_pca.png), [cluster fingerprints](../reports/figures/failure_aware_vppv/failure_aware_vppv_model_derived_cluster_fingerprints.png) | Behavior clusters are mapped to route assignments before held-out weak-label evaluation. | Simulator rollout representation, not model-internal hidden-layer analysis or real clinical data. |
