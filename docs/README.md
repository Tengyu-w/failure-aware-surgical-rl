# Documentation

Recommended reading order:

1. [Research sequence](research_sequence.md)
2. [PhD application project brief](phd_application_project_brief.md)
3. [Evidence index](evidence_index.md)
4. [SurRoL upgrade project tracker](surrol_upgrade_project.md)

Archive folders:

- `archive/planning_notes/`: early planning notes.
- `archive/upload_notes/`: GitHub upload notes used during repository
  preparation.

These archive folders are preserved for provenance, but they are not the main
review path.
