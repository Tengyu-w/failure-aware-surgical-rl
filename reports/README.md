# Reports Directory

This directory contains the core research reports and the evidence folders used
by the repository README.

Read these first:

1. [GitHub research report](../docs/RESEARCH_REPORT.md)
2. [Experiment evidence summary](../docs/EXPERIMENT_EVIDENCE_SUMMARY.md)
3. [Method overview](../docs/METHOD_OVERVIEW.md)
4. [Figure atlas](../docs/FIGURE_ATLAS.md)
5. [ECG-style RL upgrade](../docs/ECG_STYLE_RL_UPGRADE.md)
6. [ECG-style RL reliability suite](ecg_style_rl_reliability_suite.md)
7. [Multi-signal reliability upgrade](multisignal_reliability_upgrade.md)
8. [SurRoL master results](surrol_master_results.md)
9. [Fault taxonomy](surrol_fault_taxonomy_step2.md)
10. [Learned route classifier](surrol_learned_route_classifier_step3.md)
11. [Observable supervisor](surrol_observable_supervisor_step4.md)
12. [Risk-gated tangent backup](risk_gated_tangent_report.md)
13. [Failure-aware VPPV final teacher brief](failure_aware_vppv_final_teacher_brief.md)
14. [Failure-aware VPPV GitHub readiness audit](failure_aware_vppv_github_readiness_audit.md)
15. [Failure-aware VPPV model-derived routing](failure_aware_vppv_model_derived_routing.md)

## Evidence Folders

- `tables/`: CSV summaries and scored results.
- `figures/`: plots for recovery, routing, embeddings, and proxy experiments.
- `media/`: SurRoL rendered rollout GIF/MP4/PNG evidence.

The main research story is the stage-ordered report in
[docs/RESEARCH_REPORT.md](../docs/RESEARCH_REPORT.md).
