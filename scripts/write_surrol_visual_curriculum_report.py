...
RUNS = ROOT / "runs"
REPORT = ROOT / "reports" / "surrol_visual_curriculum_round27_zh.md"


def read_rows(name: str) -> list[dict[str, str]]:
    with (RUNS / name).open(newline="") as handle:
...
def main() -> None:
    sparse = read_rows("surrol_ppo_eval_needlereach_render_sparse_clean_3ep.csv")
    progress = read_rows("surrol_ppo_eval_needlereach_render_progress100_clean_3ep.csv")
    sparse_sensitivity = read_rows("surrol_visual_sensitivity_needlereach_sparse_20states.csv")
    progress_sensitivity = read_rows("surrol_visual_sensitivity_needlereach_progress100_20states.csv")
    ...
    lines = [
        ...
        "|---|---:|---:|---:|---:|---:|",
        f"| sparse (`scale=0`) | {average(sparse, 'success'):.1%} | {average(sparse, 'initial_distance'):.4f} | {average(sparse, 'min_distance'):.4f} | {average(sparse, 'final_distance'):.4f} | {average(sparse, 'net_distance_progress'):.4f} |",
        f"| progress (`scale=100`) | {average(progress, 'success'):.1%} | {average(progress, 'initial_distance'):.4f} | {average(progress, 'min_distance'):.4f} | {average(progress, 'final_distance'):.4f} | {average(progress, 'net_distance_progress'):.4f} |",
        "",
        ...
        "|---|---:|---:|---:|---:|",
        f"| sparse | {len(sparse_sensitivity)} | {average(sparse_sensitivity, 'observation_l2_delta'):.4f} | {average(sparse_sensitivity, 'action_l2_delta'):.4f} | {max(float(r['action_l2_delta']) for r in sparse_sensitivity):.4f} |",
        f"| progress-100 | {len(progress_sensitivity)} | {average(progress_sensitivity, 'observation_l2_delta'):.4f} | {average(progress_sensitivity, 'action_l2_delta'):.4f} | {max(float(r['action_l2_delta']) for r in progress_sensitivity):.4f} |",
        "",
